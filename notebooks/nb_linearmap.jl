...
# ╔═╡ ea372314-3bc5-4524-b562-bf69b40e6e50
begin
	using LinearMaps
	using SparseArrays
	using Plots
...
# ╔═╡ 473e5860-6d59-47cd-8fef-327aa09fe3ef
function D1(v, h)
	N = length(v)
	vₓ = similar(v)
...
# ╔═╡ 9761527d-3a9d-4e9f-a5f6-a53065c00233
A = LinearMap(v->D1(v,1), 100,100)

# ╔═╡ c02b42b0-c439-4f2d-8ee1-0a15626feccf
begin
	Ns = 2:10:4000
	ts = zeros(length(Ns))
	for i ∈ eachindex(Ns)
		A = LinearMap(v->D1(v,1), Ns[i])
		ts[i] = @elapsed sparse(A)
	end
end

# ╔═╡ 7a54ec3b-d925-44b3-97f9-3b38c86773a4
let
	p = fit(Ns,ts, 2)
	plot(Ns,ts;
		legend=false,
...
# ╔═╡ 6e4e878a-bcc3-43e1-b236-eb4a15cf9670
sparse(A)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "dbba5e53b8cf8b2098de8979726b3e02dae25480"
